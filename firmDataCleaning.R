...
firmData <- tbl_df(firmData) %>% filter(!number_id %in% c(998655, 12213, 58027, 10736, 23430, 148, 276, 310, 9278, 10737, 10920, 11001, 11163, 11164, 11184, 11195, 30452, 46016, 10852, 12034, 12204, 18291, 130910, 11914))##Removed 215 obs

                                                ############ SECTION: 3 ###########                                                                                                              CREATE NEW VARIABLES                                                                                                         ###################################
...
firmData <- bind_rows(firmData, temp) %>% arrange(number_id, year) %>% ungroup() #Added rows of 'ghost' observations
...
firmData <- group_by(firmData, number_id) %>% mutate(minYear = as.integer(min(year, na.rm = T))) %>% ungroup() #%>% filter(minYear >= dateofincorporation) ## dropped 0 obs

firmData <- ungroup(firmData) %>% mutate(firmType = ifelse((year == minYear & year == dateofincorporation), "Entering Firm", ifelse(year == minYear, "First Year in Ruslana", ifelse((year == exit_year + 1 & !is.na(exit_year)), "Ghost Firm", "Incumbent"))))
...
firmData <- ungroup(firmData) %>% group_by(number_id) %>% mutate(lagEmp = lag(numberofemployees, 1), missingYear = ((year - lag(year, 1)) != 1)) %>% ungroup() %>% mutate(deltaEmp = ifelse(as.character(firmType) == "Entering Firm", numberofemployees, numberofemployees - lagEmp)) %>% mutate(deltaEmp = ifelse(missingYear, NA, deltaEmp))
...
firmData <- ungroup(firmData) %>% group_by(number_id) %>% mutate(lagOperRevenue = lag(operatingrevenueturnover, 1), lagProfit = lag(operatingprofitloss, 1)) %>% ungroup() %>% mutate(diffRevenue = operatingrevenueturnover - lagOperRevenue, diffProfit = operatingprofitloss - lagProfit) %>% select(-lagOperRevenue, -lagProfit)
                                                ...
                                                ############ SECTION: 2 ###########                                                                                              ADDING THE REGULATION AND SPURT/SLUMP VARIABLES                                                                                              ###################################
##Joining the labor regulations and spurts
load("Data/Saved Datasets/regionalALL.RDA")
load("Data/Saved Datasets/isicData.RDA")
firmData <- left_join(firmData, regionAll, by = c("region", "year")) ## there are 6 regions with 4899 obs in the firm dataset with no laborreg data, the year 2011 is missing
firmData <- left_join(firmData, isicData, by = c("isic", "year")) ## the years 1999 to 2003 are missing in isic data

                                                ############ SECTION: 4 ###########                                                                                                   CREATE THE LABOR REALLOCATION DATASET                                                                                                   ###################################
reallocationData <- group_by(firmData, isic, region, year) %>% mutate(noobs = sum(!is.na(deltaEmp))) %>% filter(noobs > 5) %>% summarise(Rind = sum(abs(deltaEmp), na.rm = T)/sum(numberofemployees, na.rm = T)) %>% ungroup() %>% group_by(isic, region) %>% summarise(R = mean(Rind))
...
firmData <- ungroup(firmData) %>% select(number_id, year, exit_year, age, region, isic, firmType, grTrend, numberofemployees, lagEmp,laborreg2, laborreg8, logKperLmean, diffRevenue, diffProfit, grp, grppc, firms, fixedassets, chapter_a, chapter_c, chapter_d, chapter_l, chapter_m, unemp, wagepc, capture, deltaEmp)
